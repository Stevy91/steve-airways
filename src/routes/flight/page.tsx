...
const fetchFlightData = async (params: URLSearchParams, signal?: AbortSignal) => {
    try {
        const [locationsRes, flightAllRes, filteredFlightsRes] = await Promise.all([
            fetch(`http://localhost:3003/locations`, { signal }),
            fetch(`http://localhost:3003/flightall`, { signal }),
            fetch(`http://localhost:3003/flights?${params.toString()}`, { signal }),
        ]);
        ...
        if (params.get("trip_type") === "roundtrip" && params.get("return_date")) {
            ...
            const returnFlightsRes = await fetch(`http://localhost:3003/flights?${returnParams.toString()}`, { signal });
