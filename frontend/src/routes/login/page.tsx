...
export default function Login() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch("https://steve-airways-production.up.railway.app/api/login", {
        method: "POST",
    ...
    <div className="flex min-h-screen items-center justify-center bg-gray-100">
      <div className="w-full max-w-md rounded-2xl bg-white p-8 shadow-lg">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          >
            {loading ? "Connexion..." : <><LogIn className="mr-2 h-4 w-4" /> Connexion</>}
          </button>
        </form>

        <p className="mt-6 text-center text-xs text-gray-400">
          Ceci est un environnement prototype. Les identifiants sont uniquement
